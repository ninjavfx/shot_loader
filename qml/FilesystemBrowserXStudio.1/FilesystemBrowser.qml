...
Item {
    ...
    function selectItem(index, selectionMode, allowPreview) {

        if (allowPreview === undefined)
            allowPreview = true
        ...
        if (selectionMode === 1) { // Shift
            ...
            // A double-click emits another release after its load handler.
            // Only a genuinely new selection should arm the single-click
            // preview timer; otherwise that release replaces the loaded media
            // with an uncoloured preview a moment later.
            var selectionChanged = !selectedItems.includes(index)
            if (selectionChanged) {
                selectedItems = [index]
            }
            if (allowPreview && selectionChanged && selectedItems.length == 1) {
    ...
    // onReleased follows onDoubleClicked, and would otherwise re-arm preview.
    property bool suppressNextReleasePreview: false

    Timer {
        id: previewTimer
        // Keep this longer than the desktop double-click interval. At 200 ms a
        // preview was launched before the double-click load arrived, so the
        // preview could replace the loaded media in the viewport.
        interval: 500
        repeat: false
        onTriggered: {
                ...
                MouseArea {
                    ...
                    onReleased: (mouse)=> {
                        if (underMouseIndex != -1 && mouse.button === Qt.LeftButton && mouse.modifiers == Qt.NoModifier && !drag_drop_handler.dragging) {    
                            var allowPreview = !suppressNextReleasePreview
                            suppressNextReleasePreview = false
                            selectedItems = []
                            selectItem(underMouseIndex, 0, allowPreview)
                        } else {
                            var pt = mapToItem(root, mouse.x, mouse.y)
                    ...
                    onDoubleClicked: (mouse) => {
                        if (mouse.button === Qt.LeftButton && underMouseIndex != -1) {
                            previewTimer.stop()
                            pendingPreviewPath = ""
                            suppressNextReleasePreview = true
                            isPreviewMode = false
