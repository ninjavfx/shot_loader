...
# Python plug-in. It retains that plug-in's browser, scanning, QML, and media
# loading foundations, with Shot Loader-specific sequence/CDL handling added
# on top.

from xstudio.plugin import PluginBase
from xstudio.core import JsonStore, FrameList, add_media_atom, Uuid, UuidActor, media_source_atom
import os
import re
...
_CDL_DEBUG_LOG = os.path.join(os.path.dirname(__file__), "shot_loader.log")

def _cdl_log(message):
    """Temporary, file-based diagnostics for the asynchronous OCIO hand-off."""
    try:
        with open(_CDL_DEBUG_LOG, "a", encoding="utf-8") as stream:
            stream.write("{}\n".format(message))
    except Exception:
        pass
...
class XStudioHostInterface:
    ...
    @staticmethod
    def _sequence_stem(path):
        """Return the versioned sequence stem from a frame or fileseq path."""
        if fileseq_available:
            try:
                # ``str(FileSequence)`` may contain a compact range such as
                # ``v018.1000-1066#.exr``. basename() is the parser's stable
                # version prefix (``v018.``), avoiding regex guesses here.
                sequence_stem = fileseq.FileSequence(path).basename().rstrip(".")
                if sequence_stem:
                    return sequence_stem
            except Exception:
                pass
        filename = os.path.basename(str(path).split("=", 1)[0])
        stem, _extension = os.path.splitext(filename)
        return re.sub(
            r"(?:[._])(?:\{:\d+d\}|#+|@+|%0?\d*d|\d+)$", "", stem)

    def _find_cdl_sidecar(self, path):
        """Find the first configured ``<sequence-stem>.ccc`` sidecar file."""
        frame_dir = os.path.dirname(str(path).split("=", 1)[0])
        sequence_stem = self._sequence_stem(path)
        locations = self.plugin.cdl_search_locations

        for location in locations:
            directory = (
                frame_dir if location == "same-directory"
                else os.path.dirname(frame_dir)
            )
            candidate = os.path.join(directory, sequence_stem + ".ccc")
            if os.path.isfile(candidate):
                _cdl_log("found sidecar: {}".format(candidate))
                return candidate, sequence_stem
        _cdl_log("no sidecar for path={!r}, stem={!r}, locations={!r}".format(
            path, sequence_stem, locations))
        return None, sequence_stem

    def _apply_cdl_context(self, media, path):
        """Attach the OCIO context required by the CDL-enabled display view.

        The OCIO config resolves ``${VERSION}.ccc`` through its context-aware
        search path.  ``CDL_dir`` deliberately preserves the exact case used by
        the config; OCIO context-variable names are case-sensitive.
        """
        cdl_path, sequence_stem = self._find_cdl_sidecar(path)
        if not cdl_path:
            return

        context = {
            "CDL_dir": os.path.dirname(cdl_path),
            "VERSION": sequence_stem,
        }
        _cdl_log("queue context for {!r}: {!r}".format(path, context))
        # xStudio constructs/replaces colour-pipeline state asynchronously after
        # add_media(). A successful immediate write is not sufficient: the first
        # shader build can still happen with an empty context a few milliseconds
        # later. Keep this source's context alive while that setup settles.
        def write_context_and_check():
            source = media.media_source()
            source.set_metadata(context, "/colour_pipeline/ocio_context")
            return source, source.get_metadata("/colour_pipeline/ocio_context") == context

        def refresh(source):
            colour_pipeline = self.connection.api.app.active_viewport.colour_pipeline
            self.connection.request_receive(
                colour_pipeline.remote,
                media_source_atom(),
                UuidActor(source.uuid, source.remote))

        def retry_after_hook():
            applied_once = False
            for attempt in range(12):
                if attempt:
                    time.sleep(0.25)
                try:
                    source, applied = write_context_and_check()
                    _cdl_log("attempt {}: applied={!r}, source={}".format(
                        attempt + 1, applied, source.uuid))
                    if applied:
                        refresh(source)
                        applied_once = True
                        _cdl_log("attempt {}: colour pipeline refresh complete".format(
                            attempt + 1))
                except Exception as error:
                    _cdl_log("attempt {} failed: {!r}".format(attempt + 1, error))
            if applied_once:
                print("[shot_loader] CDL context applied: {}".format(cdl_path))
            else:
                print("[shot_loader] CDL context did not persist after media setup: {}".format(
                    cdl_path))

        threading.Thread(target=retry_after_hook, daemon=True).start()
    ...
    def load_media(self, path):
        ...
        # Set the per-source OCIO context before selecting the media. xStudio
        # reads the source colour parameters when it becomes visible.
...
class ShotLoaderPlugin(PluginBase):
    def __init__(self, connection):
        ...
        # Ordered relative locations checked for a per-shot CDL/CCC file.  This
        # is deliberately a list (rather than a single on/off setting): the
        # eventual resolver will try each location in this order for every
        # loaded sequence.
        self.cdl_search_locations_attr = self.add_attribute(
            "CDL Search Locations",
            ["same-directory", "parent-directory"],
            {"title": "CDL Search Locations", "category": "Shot Loader"},
            register_as_preference=True,
            preference_path="/plugin/shot_loader/cdl_search_locations"
        )
        self.cdl_search_locations_attr.expose_in_ui_attrs_group(
            "Shot Loader Settings")
        ...
        self.files_attr.expose_in_ui_attrs_group("Shot Loader")
        ...
        self.current_path_attr.expose_in_ui_attrs_group("Shot Loader")
        ...
        self.command_attr.expose_in_ui_attrs_group("Shot Loader")
        
        # A panel hotkey is wired by xStudio only when the panel also has a
        # native pop-out registration (see register_ui_panel_qml). The callback
        # remains a no-op because xStudio owns that window's visibility state.
        self.toggle_browser_action = self.register_hotkey(
            self.toggle_browser, # hotkey_callback
            "S", # default_keycode
            1, # ShiftModifier
            "Show Shot Loader",
            "Toggles the Shot Loader panel",
            False, # auto_repeat
            "ShotLoader", # component
            "any" # context
        )
        ...
        # Register both the dockable panel and xStudio-managed pop-out. Passing
        # the hotkey UUID here is what makes L show/hide the pop-out window.
        self.register_ui_panel_qml(
            ...
            "qrc:/icons/folder.svg",
            6.0,
            self.toggle_browser_action
        )
        ...
        self.completions_attr.expose_in_ui_attrs_group("Shot Loader")
        ...
        self.searching_attr.expose_in_ui_attrs_group("Shot Loader")
        ...
        self.progress_attr.expose_in_ui_attrs_group("Shot Loader")
        ...
        self.scanned_attr.expose_in_ui_attrs_group("Shot Loader")
        ...
        self.scanned_dirs_attr.expose_in_ui_attrs_group("Shot Loader")
        ...
        self.directory_query_result.expose_in_ui_attrs_group("Shot Loader")

        self.depth_limit_attr = self.add_attribute(
            "recursion_limit",
            self.config.get("max_recursion_depth", 6),
            {"title": "Recursion Limit", "category": "Shot Loader"},
            register_as_preference=True
        )
        self.depth_limit_attr.expose_in_ui_attrs_group("Shot Loader Settings")
        ...
        self.view_mode_attr.expose_in_ui_attrs_group("Shot Loader")
        ...
        self.scan_required_attr.expose_in_ui_attrs_group("Shot Loader")

        self.is_deepscan_attr = self.add_attribute(
            "is_deepscan",
            False
        )
        self.is_deepscan_attr.expose_in_ui_attrs_group("Shot Loader")

        self.auto_scan_threshold_attr = self.add_attribute(
            "auto_scan_threshold",
            self.config.get("auto_scan_threshold", 4),
            {"title": "auto_scan_threshold", "category": "Shot Loader"},
            register_as_preference=True
        )
        self.auto_scan_threshold_attr.expose_in_ui_attrs_group("Shot Loader Settings")
        ...
        self.filter_time_attr.expose_in_ui_attrs_group(["Shot Loader", "Filter Terms"])
        ...
        self.filter_version_attr.expose_in_ui_attrs_group(["Shot Loader", "Filter Terms"])
        ...
        self.history_attr.expose_in_ui_attrs_group("Shot Loader")
        ...
        self.pinned_attr.expose_in_ui_attrs_group("Shot Loader")
        ...
        self.extensions_attr = self.add_attribute(
            ...
            {"title": "Media Extensions", "category": "Shot Loader"},
            register_as_preference=True
        )
        self.extensions_attr.expose_in_ui_attrs_group("Shot Loader Settings")

        self.ignore_dirs_attr = self.add_attribute(
            "Ignore Directories",
            ", ".join(self.config.get("ignore_dirs", [])),
            {"title": "Ignore Directories", "category": "Shot Loader"},
            register_as_preference=True
        )
        self.ignore_dirs_attr.expose_in_ui_attrs_group("Shot Loader Settings")

        self.root_ignore_dirs_attr = self.add_attribute(
            "Root Ignore Directories",
            ", ".join(self.config.get("root_ignore_dirs", [])),
            {"title": "Root Ignore Directories", "category": "Shot Loader"},
            register_as_preference=True
        )
        self.root_ignore_dirs_attr.expose_in_ui_attrs_group("Shot Loader Settings")
        ...
        self.thumbnail_request_attr.expose_in_ui_attrs_group("Shot Loader")
    ...
    @property
    def cdl_search_locations(self):
        """Configured CDL search locations, normalized for the future resolver."""
        value = self.cdl_search_locations_attr.value()
        ...
        if not isinstance(value, list):
            return []
        valid_locations = {"same-directory", "parent-directory"}
        return [location for location in value if location in valid_locations]
    ...
    def toggle_browser_from_menu(self, menu_item=None, user_data=None):
        ...
        # Since we are now a standard dockable panel, the user should use View -> Panels -> Shot Loader
        # or rely on the hotkey's default action if it maps to the view.
        # We'll just log here.
        _dbg("Menu item clicked. The Shot Loader is available in the Panels menu.")
    ...
    def toggle_browser(self, converting, context):
        _dbg(f"Toggling Shot Loader (Action Triggered). Context: {context}")
...
def create_plugin_instance(connection):
    return ShotLoaderPlugin(connection)
